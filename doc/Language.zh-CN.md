...
　　语言实现为程序开始运行时的具有的[当前环境](#名称解析)称为*初始环境(initial environment)* 。
...
* 除非另行指定，函数值不是引用值。
* 除非另行指定，函数不在函数值中[保留引用值](#保留引用值) ，即函数值[经过返回值转换](#间接值作为函数值)。
...
* 除非另行指定，引起错误抛出[异常](#诊断)。
* 求值时引起的错误使求值中断[可产生副作用](#作用使用原则)，这样的副作用总是[后序](#作用顺序)于已被求值的表达式中产生的副作用。
* 被错误处理和检查的函数不修改参数或者函数调用外创建的对象。

## 操作类型约定

　　谓词即 [`<predicate>`](#实体元素文法约定)，是结果类型为 `<boolean>` 的函数。
...
　　创建[新](#新环境)标准环境并以这个环境为当前环境加载 `<string>` 指定的翻译单元作为源的模块。
