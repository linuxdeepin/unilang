...
　　引用值的直接或者间接的被引用对象是自身时构成循环引用。或者程序执行时引入循环引用的程序具有未定义行为。
...
**注释** 同一般对象，被引用对象的访问应在生存期内，否则程序行为未定义。
...
**注释** 引用值提升转换不传播引用值的属性。根据引用值的性质，若操作数是已被折叠的引用值，则引用值提升转换等价左值到右值转换；否则，有限次的引用值提升转换等价左值到右值转换。

**注释** 引用值提升转换不传播不可修改属性，类似 [ISO C++] 非引用值的转换在结果中不保留源操作数中的 `const` 类型。
...
　　以被求值的表达式和所在的环境作为参数，基础语言使用以下统一的*求值算法(evaluation algorithm)* ：
...
　　部分操作可能修改参数指定或关联的对象。修改的对象一般通过引用值指定。若被修改的对象以子对象引用访问，则不保证修改总是共享到它的被引用对象。
