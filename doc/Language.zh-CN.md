...
　　具有引用值的表达式是*左值(lvalue)* ，否则是*右值(rvalue)* 。左值和右值统称表达式的值类别。
...
　　初值是引用值时，初始化的引用值的被引用对象可能是初值的被引用对象，这种情形称为*引用折叠(reference collapse)* 。其它情形初始化的引用值的被引用对象是初值。是否引用折叠依初始化发生的具体上下文而定。引用折叠的结果是唯一引用的一个必要条件是被引用对象是右值引用。当前语言中的关于引用值操作中，引用值总是经过引用折叠（即不引入引用的引用）。
...
　　部分提供函数可能因使用的参数或程序的状态不同引入未定义行为。这些操作是不安全的。不安全操作是实现可选提供的。

　　不安全操作包括通过保留不保证可安全使用的引用值（如悬空引用）的操作。这些操作的结果引入这种引用值：结果是非列表的引用值，或者是保留列表元素中的引用值的列表右值。结果中的引用值来自参数且不保留引用值时，参数经过一次左值到右值转换替换为被引用对象。
