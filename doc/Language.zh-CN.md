...
　　为修改对象而直接（而不是通过指定参数求值）蕴含副作用函数名以 `!` 结尾。

**注释** 修改对象可改变对象的值。

　　若函数提供结果无条件[保留引用值](#保留引用值)和不保留引用值的不同版本且保留引用值的函数是不安全函数，保留引用值的函数名以 `%` 作为（在可能的 `!` 之前的）后缀。

　　函数名带有 `!` 后缀的函数的调用的求值可能因为不同的操作数的影响，可能视条件确定是否存在被改变的对象：

* 当通过操作数指定的条件确定是否修改时。
* 当通过操作数确定被修改的对象时。
* 当通过操作数确定引起修改的对象时。

　　带有 `!` 后缀的函数名的[调用结果](#合并子)是没有附加限制条件的*未指定值(unspecified value)* 。
