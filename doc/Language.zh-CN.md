...
　　两个对象可作为*元素(element)* 构成*有序对(ordered pair, pair)* 对象。有序对的元素是[子对象](#对象的所有权和子对象)。
...
**注释** 推论：同一个列表的元素不是另一个元素的子对象；不同元素之间不具有[所有权](#对象的所有权和子对象)，生存期不相交。
...
　　一个环境引用可能是*强引用(strong reference)* 或*弱引用(weak reference)* 之一。关联到相同的环境对象的环境强引用对关联的环境对象具有共享的[所有权](#对象的所有权和子对象)。
