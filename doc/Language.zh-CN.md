...
　　特定的值是*引用值(reference value)* ，具有引用类型。引用值关联到*被引用对象(referent)* 。引用类型和其它对象的类型不同。但除非另行指定，访问引用值总是等价于访问其被引用对象。类似 [ISO C++] ，这称为*左值到右值转换(lvalue-to-rvalue conversion)* 。例外而不进行这种转换的上下文中，引用值被保留。

　　引用值可以是唯一引用（类似 ISO C++ 的右值引用）。非引用值的唯一引用标签被忽略。
