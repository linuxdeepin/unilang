...
　　函数返回（在语言中允许出现的）间接值时，保留返回值的间接值。

　　函数在环境中绑定间接值或包含（在语言中允许出现的）间接值（如列表中以一等环境作为元素）作为子对象的对象时，保留环境的间接值。

　　函数返回包含间接值的对象由（在语言中允许出现的）参数决定时，保留参数的间接值。
...
　　[直接保留引用值](#保留间接值)覆盖[常规函数约定](#常规函数约定)的实际参数约定，指定参数不隐含[左值到右值转换](#引用值和初始化)，可按引用值直接访问。
...
　　用户程序应自行保证加载的其它模块具有足够的生存期，以避免访问悬空引用导致的未定义行为，特别是使用在环境中保留引用值的操作的情形。
...
**注释** 绑定临时对象外不和 [ISO C++] 一样可能延长右值类类型子对象的生存期。
