...
**注释** 仅在没有歧义时使用简称。应注意和更一般的实现环境概念区分。
...
　　绑定算法应确定和当前处理的操作数树的属性，即*操作数属性(operand property)* 。其中蕴含的表示操作数的项对应的[标签](#对象的其它元数据)，称为*操作数标签(operand tags)* 。
...
* 否则，匹配的子项是这个引用值的[被引用对象](#引用值)，子项的操作数标签以操作数子项中的[引用值的属性](#引用值的属性和子类型)和当前操作数标签按以下引用项继承约束限定：
	* 子项的操作数标签不包含[临时对象标签](#对象的其它元数据)。
		* **原理** 这使实现能避免临时对象的子对象具有[临时对象标签](#绑定临时对象)。
	* 子项的操作数标签是否包含[唯一引用标签](#对象的其它元数据)同引用值的属性。
