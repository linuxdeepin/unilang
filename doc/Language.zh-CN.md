...
2. [名称解析](#名称解析)：若被求值的表达式是一个符号，则这个符号被视为*变量名(variable name)* ，求值结果是该符号在上下文（[当前环境](#名称解析)确定的词法作用域）中变量绑定确定的对象的经引用折叠的引用值。
3. 否则：
	**注释** 被求值的表达式是有序对。
...
* 若有序对操作数的非前缀元素为空列表的引用值，则有序对操作数构成列表。这不会匹配形式参数树中的符号而不被绑定。
...
　　在环境中定义另一个环境中的同名变量，使被定义的变量是后者的引用值或值的[副本](#实体的副本)，则称指定此变量名的符号在后者被*导入(import)* 前者。
...
**注释** 和形式参数树中的[结尾列表](#非递归绑定)的符号被视为整体不同，递归绑定可包含项和其子项的多次递归的匹配。
