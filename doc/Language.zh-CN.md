...
　　程序可通过*引发(raise)* *错误对象(error object)* 指定引起诊断。
...
* 以翻译单元为单位进行翻译，满足：
	...
	* 对*不合式的(ill-formed)* 程序，若不存在[未定义行为](#术语和定义)，则要求翻译提供诊断消息并拒绝接受程序而终止程序的运行。（对 AOT 翻译实现，则直接终止翻译。）
...
　　除非另行指定，若实体的副本无法被创建，引起创建副本的操作[引起错误](#诊断)。
...
　　特定的对象可具有与之关联的不同对象作为它的*子对象(subobject)* 。
...
　　函数是合并子右值、合并子左值（引用值）或求值为这些值的表达式。
...
　　类型检查失败时，引起错误，称为*类型错误(type error)* 。类型错误[引发错误对象](#诊断)。
