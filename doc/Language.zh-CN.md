...
　　空列表和非空列表统称*列表(list)* ，其中包含的子对象是列表的元素。

　　一般地，列表是元素的序列。*空列表(empty list)* 是不含有元素的列表。未求值的非空列表中，子表达式是列表的元素。

　　*真列表(proper list)* 是元素有限的列表。

　　每个真列表和有限个*有序对(pair)* 的嵌套组合表示同构。每个有序对具有两个元素。

**注释** 一些编程语言中，构造有序对的操作称为 `cons` ，有序对又称为 cons 对。

　　有序对的第二个元素是列表时，则有序对是（非空的）真列表。否则，有序对是*非真列表(improper list)* 。

　　*广义列表(generalized list)* 是真列表或非真列表。

　　广义列表的元素是[一等对象](#一等实体)，是广义列表对象的[子对象](#对象的所有权和子对象)。

　　取得嵌套的有序对的广义列表的同构表示*分解(decompose)* 这个广义列表。广义列表分解后若得到有序对，可继续被分解。经有限次分解后，广义列表不再可取得不同的同构表示，即被完全分解。

　　广义列表对元素具有所有权。同一个广义列表的不同元素之间不具有所有权，[生存期](#对象的存储期和生存期)不相交。
...
**注释** 符号、空列表和有序对同时作为未求值和求值的表达式的值，被求值算法依赖，也可作为一等对象的值，是 Unilang 中的最基本的类型。其中，非真列表的不在翻译单元直接提供表达方式，而需要求值表达式得到。
